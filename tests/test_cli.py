...
def test_restore_path_mode_uses_sql_file_and_force_skips_confirmation(
    ...
    monkeypatch.chdir(tmp_path)
    monkeypatch.setattr(restore_command, "container_exists", lambda name: True)
    monkeypatch.setattr(restore_command, "container_is_running", lambda name: True)
    monkeypatch.setattr(restore_command, "current_database_size", lambda *args, **kwargs: 1024)
    monkeypatch.setattr(
        restore_command,
...
def test_restore_stopped_container_logs_and_returns(
    ...
    monkeypatch.chdir(tmp_path)
    monkeypatch.setattr(restore_command, "container_exists", lambda name: False)
...
def test_restore_remote_selection_reprompts_and_cleans_temp_download(
    ...
    monkeypatch.setattr(restore_command, "container_exists", lambda name: True)
    monkeypatch.setattr(restore_command, "container_is_running", lambda name: True)
    monkeypatch.setattr(restore_command, "current_database_size", lambda *args, **kwargs: 100)
